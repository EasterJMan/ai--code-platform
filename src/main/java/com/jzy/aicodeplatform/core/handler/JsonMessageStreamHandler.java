...
public class JsonMessageStreamHandler {
    ...
    private String handleJsonMessageChunk(String chunk, StringBuilder chatHistoryStringBuilder, Set<String> seenToolIds) {
        ...
        switch (typeEnum) {
            ...
            case TOOL_EXECUTED -> {
                ...
                String relativeFilePath = jsonObject.getStr("filePath");
                String suffix = FileUtil.getSuffix(relativeFilePath);
                String content = resolveToolFileContent(jsonObject);
    ...
    /**
     * 工具参数中的文件正文（展示用）。主字段为 content；若存在历史数据中的 contentBase64 则解码展示。
     */
    private static String resolveToolFileContent(JSONObject jsonObject) {
        String plain = jsonObject.getStr("content");
        if (StrUtil.isNotBlank(plain)) {
            return truncateForDisplay(plain);
        }
        String b64 = jsonObject.getStr("contentBase64");
        if (StrUtil.isBlank(b64)) {
            return "";
        }
        try {
            byte[] raw = Base64.getDecoder().decode(b64.replaceAll("\\s+", ""));
            return truncateForDisplay(new String(raw, StandardCharsets.UTF_8));
        } catch (IllegalArgumentException e) {
            return "[contentBase64 解码失败]";
        }
    }
