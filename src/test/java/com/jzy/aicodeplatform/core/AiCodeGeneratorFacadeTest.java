...
class AiCodeGeneratorFacadeTest {
    ...
    void generatorAndSaveCode() {
        File file = aiCodeGeneratorFacade.generateAndSaveCode("做一个交流留言板，代码不超过20行", CodeGenTypeEnum.MULTI_FILE,1L);
    ...
    void generateVueProjectCodeStream() {
        Flux<String> codeStream = aiCodeGeneratorFacade.generateAndSaveCodeStream("简单的任务记录网站,代码不超过200行", CodeGenTypeEnum.VUE_PROJECT,1L);
        // 阻塞等待所有数据收集完成
        List<String> result = codeStream.collectList().block();
        // 验证结果
        Assertions.assertNotNull(result);
